...
def profit_data(year=2017, top=25,
    ...
    if top == 'all':
        ...
        for idx in range(1, int(pages)):
            df = pd.concat([df, _dist_cotent(year, idx, retry_count,
...
def fund_holdings(year, quarter,
    ...
    for idx in range(1, pages):
        # df = df.append(_holding_cotent(start, end, idx, retry_count, pause), ignore_index=True)
        df = pd.concat([df, _holding_cotent(start, end, idx,
    ...
    try:
        ...
        if len(date_range) > 261:
            ...
            for date in date_range:
                data = data.append(
                    _sz_hz(str(date.date()), retry_count, pause))
    except:
